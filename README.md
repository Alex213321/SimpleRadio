...
**不要在压缩包预览窗口中直接运行 EXE，也不要只把 EXE 单独复制出去。** 完整运行单位是整个程序文件夹。可以为 EXE 创建桌面快捷方式：右键点击 `SimpleRadio.exe` →“创建快捷方式”→ 将生成的快捷方式复制到桌面；此快捷方式可自由重命名并固定到任务栏。其他文件留在原目录，不要直接把 `SimpleRadio.exe` 复制到桌面，否则无法打开。

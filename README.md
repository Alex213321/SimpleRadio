...
1. 在本地新建一个空白文件夹，例如 `D:\SimpleRadio`。你可以根据自己希望保存和使用播放器的位置自由选择路径与文件夹名称；建议为下载、解压和缓存预留至少 **6 GB** 可用空间。
2. 在 [Releases](https://github.com/Alex213321/SimpleRadio/releases/tag/v1.4.2) 页面中，点击下载 `SimpleRadio-1.4.2-Music.zip` 和 `SimpleRadio-1.4.2-Program-x64.zip`，并把这两个压缩包保存到刚才新建的文件夹中。
3. 在当前文件夹中分别解压这两个压缩包，也就是选择“解压到此处”。**两个压缩包一定要解压到同一个文件夹中**，这样解压后会自动合并为同一个 `SimpleRadio-1.4.2` 文件夹。进入该文件夹后可以看到 `SimpleRadio.exe`，双击即可启动使用。
...
5. 第一次双击启动应用时，Windows 可能提示“已保护你的电脑”或“阻止了无法识别的应用启动”。这是因为当前版本尚未进行代码签名，并不代表程序一定存在问题。请先确认文件来自本仓库的官方 Release，并可对照 `SHA256SUMS.txt` 核对完整性；确认来源无误后，点击“更多信息”→“仍要运行”即可。此提示通常只会在首次启动时出现；如果文件来源不明或校验结果不一致，请不要运行。
6. **温馨提示：**如果你觉得前述过程比较麻烦，也可以通过 [夸克网盘](https://pan.quark.cn/s/d82a16c6a926) 下载单一完整压缩包，提取码为 `KgHe`。下载并完整解压后即可直接使用。
